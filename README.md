...
An interactive, marker-based Augmented Reality application developed for the **U.S. Army Corps of Engineers (USACE)**. This project serves as an educational exhibit designed to teach the public—targeting 10,000+ views—about the environmental impacts of stormwater runoff, pollution, and the critical importance of wetland conservation.

*(Note: See the `/Media` folder for demo videos, and `/Documentation` for full engineering specifications).*
...
*Above: High-level visualization of the integration between AR Foundation surface detection, marker tracking, and the application's interactive event system.*
...
* **Interactive Event Systems:** Developed modular C# scripts to handle user inputs, triggering synchronized animations (e.g., bulldozers, cranes, trash movement) and state changes to simulate environmental degradation and restoration.
...
* `/Media` - Contains video demonstrations of the AR application in action.
